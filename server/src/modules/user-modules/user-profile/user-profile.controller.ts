...
import { ApiPayloadUpdateUserProfile } from './api/payload/update-profile';
...
export class UserProfileController {
  ...
  @Get('basic')
  async getUserProfileBasic(@Param() { user_id }: ApiParamUser) {
    return this.service.getBasicProfile(user_id);
  }

  @Get('details')
  @UseGuards(CookieAuthGuard, ResourceOwnerGuard)
  async getUserProfileDetails(@Param() { user_id }: ApiParamUser) {
  ...
  async updateUserAddress(
    @Param() { user_id }: ApiParamUser, // @Body()
  ) {}
}
